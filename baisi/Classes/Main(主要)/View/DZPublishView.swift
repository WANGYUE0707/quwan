...
var puWindow: UIWindow! = nil
class DZPublishView: UIView {
    
    
    class func publishView() -> DZPublishView {
        
       return NSBundle.mainBundle().loadNibNamed("DZPublishView", owner: nil, options: nil).last as! DZPublishView
    }
    
//    init(nibName: String) {
//        super.init()
//    let view = NSBundle.mainBundle().loadNibNamed("DZPublishView", owner: nil, options: nil).last as! DZPublishView
//        
//    }
//
...
//    }
    ...
    class func show() {
        ...
        puWindow = UIWindow(frame: UIScreen.mainScreen().bounds)
        puWindow.backgroundColor = UIColor.whiteColor().colorWithAlphaComponent(0.8)
        puWindow.hidden = false
        
        let view = DZPublishView.publishView()
        view.frame = puWindow.bounds
        puWindow.addSubview(view)
//        self.puWindow = puWindow
    }
    ...
    func cancelPublish(completion: completionClosure?) {
        ...
        for i in 1..<subviews.count {
            ...
            animation.beginTime = CACurrentMediaTime() + Double(i - 1) * Double(DZAnimationDelay)
            subView.pop_addAnimation(animation, forKey: nil)
            
            if i == subviews.count - 1 {
                animation.completionBlock = { [weak self] pop, finshed  in
